...
async def get_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not parsed_date:
        await update.message.reply_text(
            "❌ Invalid date format. Please use <b>DD/MM/YYYY</b> (e.g., 20/08/2026):",
            parse_mode="HTML",
        )
        return DATE
    ...
    await update.message.reply_text(
        "⏰ Enter time in 24-hour format <b>HH:MM</b> (e.g. <i>14:30</i>):",
        parse_mode="HTML",
    )
    return TIME
...
async def get_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Invalid time format. Use 24-hour format <b>HH:MM</b> (e.g., 09:15 or 18:30):",
            parse_mode="HTML",
        )
        return TIME
    ...
    if local_dt < now_local:
        await update.message.reply_text(
            "⚠️ You cannot set a reminder in the past! Please re-enter a future date or time.",
            parse_mode="HTML",
        )
        return TIME
    ...
    await update.message.reply_text(
        "📝 Enter optional description or details (or tap Skip):",
...
async def get_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("One-time", callback_data="REC_none"),
...
async def save_reminder_callback(
            ...
            [
                [
                    InlineKeyboardButton(
                        "📅 View Reminders", callback_data="NAV_REMINDERS"
                    ),
                    InlineKeyboardButton(
                        "🏠 Main Menu", callback_data="NAV_MAIN_MENU"
                    ),
                ]
            ]
        ),
...
async def list_reminders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reminders:
        ...
        for rem in reminders[:10]:
            dt = datetime.fromisoformat(rem["reminder_datetime"]).strftime(
                "%d %b %Y, %H:%M"
            )
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        f"✅ Complete #{rem['id']}",
                        callback_data=f"REM_DONE_{rem['id']}",
                    ),
                    InlineKeyboardButton(
                        f"🗑️ Delete #{rem['id']}",
                        callback_data=f"REM_DEL_{rem['id']}",
                    ),
                ]
            )
        ...
        buttons.append(
            [
                InlineKeyboardButton("➕ Add New", callback_data="NAV_ADD"),
                InlineKeyboardButton("🏠 Main Menu", callback_data="NAV_MAIN_MENU"),
            ]
        )
...
add_reminder_conv = ConversationHandler(
    ...
    states={
        ...
        CONFIRM: [
            CallbackQueryHandler(save_reminder_callback, pattern="^CONFIRM_")
        ],
    },
